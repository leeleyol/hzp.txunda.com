...
class PostCommentLogic extends BaseLogic {
    ...
    function getList($request = array()) {
        ...
        if(!empty($request['title'])){
            $param['where']['pc.title']  = array('like','%'.$request['title'].'%');
        }
        $param['where']['pc.post_id']   = $request['post_id'];
        $param['where']['pc.status']   = array('lt',9);        //状态
        $param['order']             = 'create_time DESC';   //排序
        $param['page_size']         = C('LIST_ROWS');       //页码
        $param['parameter']         = $request;             //拼接参数

        $result = D('PostComment')->getList($param);
    ...
    function findRow($request = array()) {
        if(!empty($request['id'])) {
            $param['where']['pc.id'] = $request['id'];
        } else {
            $this->setLogicError('参数错误！'); return false;
        }
        $param['where']['pc.status'] = array('lt',9);
        $row = D('PostComment')->findRow($param);
