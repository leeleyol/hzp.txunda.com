...
class PostLogic extends BaseLogic {
    ...
    function getList($request = array()) {
        ...
        if(!empty($request['title'])){
            $param['where']['p.title']  = array('like','%'.$request['title'].'%');
        }
        $param['where']['p.status']   = array('lt',9);        //状态
        $param['order']             = 'create_time DESC';   //排序
        $param['page_size']         = C('LIST_ROWS');       //页码
        $param['parameter']         = $request;             //拼接参数

        $result = D('Post')->getList($param);
    ...
    function findRow($request = array()) {
        if(!empty($request['id'])) {
            $param['where']['p.id'] = $request['id'];
        } else {
            $this->setLogicError('参数错误！'); return false;
        }
        $param['where']['p.status'] = array('lt',9);
        $row = D('Post')->findRow($param);
