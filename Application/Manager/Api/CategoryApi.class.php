...
class CategoryApi {
    ...
    function handleOperateOne($list = array(), $model = '', $prohibit = array(), $only_allow = array('add_child','edit','set_status','delete')) {
        //空数组返回空
        if(empty($list)) { return array(); }
        //
        foreach($list as $key => $value) {
    ...
    function getSelect($list = array(), $field_name = '', $index_value = '', $index_field = 'id') {
        ...
        //下拉模板
        $template = "<li data-value='{id}' data-title='{top_class}{type_name}'><a href='javascript:void(0)'>{top_class}{name}</a></li>";
        //设置初始数据
    ...
    function getSelectOne($list = array(), $field_name = '', $index_value = '', $index_field = 'id') {
        //判空
        if(empty($list)) return '';
        //将数据转换成树状结构
        $tree_data = list_to_tree($list);
        //下拉模板
        $template = "<li data-value='{id}' data-title='{top_class}{type_name}'><a href='javascript:void(0)'>{top_class}{type_name}</a></li>";
        //设置初始数据
        api('Tree/init',array($tree_data,$template,array('id','type_name')));
        //生成树状下拉菜单
        $html = api('Tree/toSelectFormatTree',array($index_value, $index_field));
        //html结构
        $select = "<div class='btn-group'>
