 ...
 * 商品分类
 */
class CategoriesLogic extends BaseLogic {
    ...
    function getList($request = array()) {
        $param['where']['depth'] = array('in','1');
        $param['where']['status']   = array('lt',9);        //状态
        $param['order']             = 'create_time DESC';   //排序
        $param['page_size']         = 4;        //页码
        $param['parameter']         = $request;             //拼接参数

        $result = D('Categories')->getList($param);
    ...
    function findRow($request = array()) {
        ...
        $row = D('Categories')->findRow($param);
    ...
    function setStatus($request = array()) {
        ...
        $arr =  M($request['model'])->where(array('p_id'=>array('in',$request['ids']),'status'=>array('neq','9')))->select();
        if($arr){
