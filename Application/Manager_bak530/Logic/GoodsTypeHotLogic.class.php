 ...
 * 分类关键字管理逻辑层
 */
class GoodsTypeHotLogic extends BaseLogic {
    ...
    function getList($request = array()) {
        if($_REQUEST['keywords']){
            $param['where']['gth.keywords'] = array('LIKE','%'.$_REQUEST['keywords'].'%');
        }
        if($_REQUEST['cid']){
            $param['where']['gth.cid'] = $_REQUEST['cid'];
        }

        $param['order']             = 'cid ASC';   //排序
        $param['where']['status']   = array('lt',9);        //状态
        $param['page_size']         = C('LIST_ROWS');        //页码
        $param['parameter']         = $request;             //拼接参数
        $result = D('GoodsTypeHot')->getList($param);
        return $result;
    }
    ...
    function findRow($request = array()) {
        ...
        $row = D('GoodsTypeHot')->findRow($param);
        if(!$row) {
